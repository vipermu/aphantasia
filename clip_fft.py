...
def get_args():
    ...
    parser.add_argument('-o',
                        '--overscan',
                        action='store_true',
                        help='Extra padding to add seamless tiling')
    ...
    parser.add_argument('-e',
                        ...
                        default=0,
                        type=float,
                        help='Enhance consistency, boosts training')
    parser.add_argument('-n',
                        '--noise',
                        default=0,
                        type=float,
                        help='Add noise to suppress accumulation')  # < 0.05 ?
    parser.add_argument(
        '-nt',
        '--notext',
        default=0,
        type=float,
        help='Subtract typed text as image (avoiding graffiti?), [0..1]')
...
def to_valid_rgb(image_f, colors=1., decorrelate=True):
    color_correlation_svd_sqrt = np.asarray([[0.26, 0.09, 0.02],
                                             ...
                                             [0.27, -0.09,
                                              0.03]]).astype("float32")
    color_correlation_svd_sqrt /= np.asarray([colors, 1.,
                                              1.])  # saturate, empirical
...
def fft_image(shape, sd=0.01, decay_power=1.0, resume=None):  # decay ~ blur
    ...
    init_val_size = (b, ch) + freqs.shape + (
        2, )  # 2 for imaginary and real components

    if resume is None:
        spectrum_real_imag_t = (torch.randn(*init_val_size) *
                                sd).cuda().requires_grad_(True)
    ...
    for i, img in enumerate(imgs):
        ...
        for c in range(count):
            if micro is True:  # both scales, micro mode
                csize = map(rnd_size[c], size // 4,
                            max(size, 0.25 * sz_min[i])).int()
